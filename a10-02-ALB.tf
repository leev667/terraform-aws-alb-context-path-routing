...
# Target Groups
  ...
  # Target Group-1: a2tg 
   a2tg = {
      ...
      tags = local.common_tags # Target Group Tags 
    } # END of Target Group-2: a2tg
  } # END OF target_groups
  tags = local.common_tags # ALB Tags
}# End of alb module

# a1tg: LB Target Group Attachment
resource "aws_lb_target_group_attachment" "a1tg" {
  for_each = {for k,v in module.ec2_private_app1: k => v}
  target_group_arn = module.alb.target_groups["a1tg"].arn
  target_id        = each.value.id
  port             = 80
}
 
# a2tg: LB Target Group Attachment
resource "aws_lb_target_group_attachment" "a2tg" {
  for_each = {for k,v in module.ec2_private_app2: k => v}
  target_group_arn = module.alb.target_groups["a2tg"].arn
  target_id        = each.value.id
  port             = 80
}
